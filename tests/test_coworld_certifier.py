...
def test_replay_coworld_starts_replay_container_and_reports_link(
    ...
    monkeypatch.setattr("cogames.coworld.play.assert_docker_image_reachable", noop_assert_docker_image_reachable)
    monkeypatch.setattr("cogames.coworld.play._free_local_port", lambda: 1234)
    monkeypatch.setattr("cogames.coworld.play._wait_for_health", noop_wait_for_health)
    monkeypatch.setattr("cogames.coworld.play.subprocess.Popen", fake_popen)
    monkeypatch.setattr(
        "cogames.coworld.play.subprocess.run",
...
def _example_root() -> Path:
    return Path(__file__).resolve().parents[1] / "src" / "cogames" / "coworld" / "examples" / "paintarena"
