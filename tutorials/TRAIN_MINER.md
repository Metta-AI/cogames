...
from cogames.games.cogs_vs_clips.missions.tutorial import MinerRewardsVariant, make_tutorial_mission
...
- **Tutorial variant**: No clips pressure and tutorial role stations
- **Miner rewards**: Dense shaping for gear pickup, extraction, and deposits
...
action_fields = type(env_cfg.game.actions).model_fields
print(f"Actions: {[name for name in action_fields if getattr(env_cfg.game.actions, name).enabled]}")
...
```
    ...
    Saved to ./train_dir/tutorial_miner.pt
